...
def test_predict_game_score_when_guest_season_is_none_should_raise_value_error(test_service):
    ...
    with pytest.raises(ValueError) as err:
        test_service.predict_game_score(guest_name, guest_season_id, host_name, host_season_id)

    # Assert
    test_service.team_season_repository.get_team_season_by_team_and_season.assert_called_once_with(guest_name, guest_season_id)
    assert err.value.args[0] == f"No season data found for '{guest_name}' in year {guest_season_id}"


def test_predict_game_score_when_host_season_is_none_should_raise_value_error(test_service):
    # Arrange
    guest_name = "Guest"
    guest_season_id = 1920
    ...
    test_service.team_season_repository.get_team_season_by_team_and_season.assert_has_calls([
        call(guest_name, guest_season_id),
        call(host_name, host_season_id),
    ])
    assert err.value.args[0] == f"No season data found for '{host_name}' in year {host_season_id}"
...
def test_predict_game_score_when_guest_season_and_host_season_are_not_none_should_return_correctly_calculated_prediction(
    ...
    guest_name = "Guest"
    guest_season_id = 1
    guest_season = TeamSeason(team_id=1, season_id=guest_season_id, league_id=league_id)
    ...
    host_name = "Host"
    host_season_id = 1
    host_season = TeamSeason(team_id=2, season_id=host_season_id, league_id=league_id)
    ...
    # Act
    game_prediction = test_service.predict_game_score(guest_name, guest_season_id, host_name, host_season_id)
