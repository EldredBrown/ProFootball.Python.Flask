...
def test_strategy(fake_team_repository, fake_team_season_repository):
    test_strategy = SubtractGameStrategy(
...
def test_process_game_when_game_arg_is_none_should_raise_value_error(test_strategy):
    ...
    with pytest.raises(ValueError) as err:
        ...
        # Assert
        assert err.value.args[0] == f"{type(SubtractGameStrategy).__name__}.process_game: game"
...
def test_process_game_when_guest_and_host_seasons_found_should_update_team_seasons_with_correct_data(
    ...
    fake_guest_season.games = 3
    fake_guest_season.wins = 1
    fake_guest_season.losses = 1
    fake_guest_season.ties = 1
    fake_guest_season.points_for = points
    fake_guest_season.points_against = points

    fake_host_season = MagicMock(TeamSeason)
    fake_host_season.games = 3
    fake_host_season.wins = 1
    fake_host_season.losses = 1
    fake_host_season.ties = 1
    fake_host_season.points_for = points
    fake_host_season.points_against = points
    ...
    assert fake_guest_season.games == 2
    ...
    assert fake_guest_season.points_for == points - guest_score
    assert fake_guest_season.points_against == points - host_score

    assert fake_host_season.games == 2
    ...
    assert fake_host_season.points_for == points - host_score
    assert fake_host_season.points_against == points - guest_score
