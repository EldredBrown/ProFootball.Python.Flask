...
def get_offensive_rankings(season_year: Optional[int], league_id: Optional[int]) -> List[OffensiveRankingsTeamSeason]:
    if season_year is None or league_id is None:
        return []

    results = sqla.callproc(f"EXEC dbo.sp_GetRankingsOffensive @season_year = {season_year}, @league_id = {league_id};")
...
def get_defensive_rankings(season_year: Optional[int], league_id: Optional[int]) -> List[DefensiveRankingsTeamSeason]:
    if season_year is None or league_id is None:
        return []

    result = sqla.callproc(f"EXEC dbo.sp_GetRankingsDefensive @season_year = {season_year}, @league_id = {league_id};")
...
def get_total_rankings(season_year: Optional[int], league_id: Optional[int]) -> List[TotalRankingsTeamSeason]:
    if season_year is None or league_id is None:
        return []

    result = sqla.callproc(f"EXEC dbo.sp_GetRankingsTotal @season_year = {season_year}, @league_id = {league_id};")
...
def get_data_for_rankings_update(team_season: TeamSeason) -> dict:
