...
def create() -> Response | str:
    ...
    if form.validate_on_submit():
        try:
            new_conference = _get_model_from_form(form)
            conference_repository = injector.get(ConferenceRepository)
...
def edit(id: int) -> Response | str:
    conference_repository = injector.get(ConferenceRepository)
    conference = conference_repository.get_conference(id)
    old_conference = copy.deepcopy(conference)
    if old_conference:
        form = EditConferenceForm()
        if form.validate_on_submit():
            try:
                new_conference = _get_model_from_form(form, id)
            ...
            _get_form_data_from_model(form, old_conference)
            if form.errors:
                flash(f"{form.errors}", 'danger')
...
def delete(id: int) -> Response | str:
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            return render_template('conferences/delete.html', conference=conference, form=form)
    except IndexError:
        abort(404)


def _get_form_data_from_model(form: ConferenceForm, conference: Conference) -> None:
    form.short_name.data = conference.short_name
    form.long_name.data = conference.long_name
    form.league_name.data = conference.league.short_name
    form.first_season_year.data = conference.first_season_id
    form.last_season_year.data = conference.last_season_id
...
def _get_model_from_form(form: ConferenceForm, id: int=None) -> Conference:
